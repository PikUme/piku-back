...
public class SignupExceptionHandler {
    ...
    public ResponseEntity<ProblemDetail> signup(SignupFlowException error, HttpServletRequest request) {
        HttpStatus status = switch (error.getReason()) {
            ...
            case PROOF_ALREADY_USED, AGREEMENT_VERSION_MISMATCH, EMAIL_ALREADY_REGISTERED, NICKNAME_COLLISION -> HttpStatus.CONFLICT;
        ...
        String detail = switch (error.getReason()) {
            ...
            case EMAIL_ALREADY_REGISTERED -> "이미 등록된 계정입니다. 기존 로그인 방법을 이용해주세요.";
    ...
    private ResponseEntity<ProblemDetail> response(String code, HttpStatus status, String detail, HttpServletRequest request) {
        ...
        if ("PROOF_EXPIRED".equals(code) || "LEGACY_SIGNUP_DISABLED".equals(code)
            || "INVALID_EMAIL".equals(code)) problem.setProperty("nextAction","AUTHENTICATE");
