...
기존 `(provider, subject)`가 있으면 같은 User로 로그인한다. 신규 subject의 Gmail 자동 연결, 기존 로그인 후 명시적 연결, 제공자 이메일 필수 검증과 신규 증명 생성은 User가 소유한다. 신규 가입에서 외부 이메일을 수용해도 자동 연결의 email_verified·authoritative 조건을 완화하지 않는다.
...
인증 증명·코드·OAuth 요청의 만료는 필요한 행 잠금을 획득한 뒤의 시각으로 판단한다. 이메일 재발송은 challenge·발송 제한 잠금을 얻은 실제 예약 시각을 유효성, 발송 횟수, 재발송 대기와 코드 만료에 함께 사용한다. 예약 후 challenge 검증에 실패하면 같은 트랜잭션의 제한 카운터 변경도 롤백한다.
...
OAuth 연결 저장과 요청 완료 기록은 별도 커밋이다. 연결 저장 후 완료 기록이 실패하면 요청이 `FAILED` 또는 `PROCESSING`으로 남을 수 있고, 완료 기록 응답만 유실되면 `CONSUMED`일 수 있다. 요청 상태로 연결 롤백을 추정하지 않는다. 같은 state는 재사용하지 않으며 새 state·nonce로 같은 subject를 다시 인증해 이미 커밋된 회원으로 복구한다.
...
V16–V18은 기존 회원을 `COMPLETED`로 보존하고 이메일 가입·동의·프로필 자료 구조를 추가한다. Google 변경의 V19가 외부 계정 테이블·제공자 메타데이터·OAuth 요청 저장소를 추가한다. 기존 동의나 Google 연결은 일괄 생성하지 않는다. 구 가입은 전환 기간에만 호출자 결속 증명으로 보호하며 신규 가입을 켜면 종료한다.
