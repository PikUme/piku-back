...
인증 증명·코드·OAuth 요청의 만료는 필요한 행 잠금을 획득한 뒤의 시각으로 판단한다. 이메일 재발송은 증명·challenge·발송 제한 잠금을 얻은 실제 예약 시각을 유효성, 발송 횟수, 재발송 대기와 코드 만료에 함께 사용한다. 예약 후 증명이나 challenge 검증에 실패하면 같은 트랜잭션의 제한 카운터 변경도 롤백한다.
