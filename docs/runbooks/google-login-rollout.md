...
챕터형 가입의 V16–V18·동의·프로필·공통 세션 기능이 먼저 반영되어야 한다. Google PR은 V19와 Google 프로토콜을 추가한다. 코드 배포만으로 기능을 열지 않으며 `signup.google.enabled`는 기본 false다. 신규 가입의 `signup.enabled`와 별도 스위치다. 실제 환경 연동 확인과 활성화는 PR 분리 작업에 포함하지 않는다.
...
1. 선행 가입 스키마와 프로필 접근 제어를 확인한 뒤 V19 OAuth 요청·제한 저장소를 적용한다.
