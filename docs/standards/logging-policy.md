...
요청 ID 전파는 모든 비동기 실행에 자동 적용되지 않는다. 현재 지원 범위는 다음과 같다.

| 실행 경계 | 전파 방식 |
| --- | --- |
| 최초 HTTP 요청과 `ASYNC`·`ERROR` 재디스패치 | 요청 속성의 ID를 사용하고 해당 디스패치 동안 MDC에 설정한다. |
| Spring MVC `Callable` 및 `WebAsyncTask`의 작업 본문 | 작업 실행 전에 ID를 설정하고 정상·예외 종료 후 작업 스레드의 이전 문맥을 복원한다. |
| SSE 완료·타임아웃·오류 콜백 | 연결 생성 시 캡처한 ID를 콜백 실행 동안 사용한다. |
| Discord 알림과 이미지 생성의 외부 HTTP 로그 콜백 | 외부 요청을 시작할 때 캡처한 ID를 해당 콜백 실행 동안 사용한다. |

`ASYNC` 재디스패치 지원은 그 전에 다른 스레드에서 수행한 모든 작업까지 자동 추적한다는 뜻이 아니다. `@Async`, `CompletableFuture`, 직접 사용하는 실행기, `DeferredResult` 결과를 만드는 작업, 새 Reactor 연산자와 타임아웃·완료 콜백을 추가할 때는 각 실행 경계의 문맥 전파를 별도로 확인한다.

새로운 비동기 작업은 요청 스레드에서 문맥을 먼저 캡처하고, 실제 작업 스레드에서 실행 범위 동안 열었다가 정상·예외 종료 시 닫는다. 기존 `RequestIdContext.capture()`와 `wrap(...)` 또는 `open()`을 기술 어댑터 경계에서 사용한다. 여러 작업에 공통 실행기를 도입한다면 `TaskDecorator` 등으로 전파를 일관되게 적용하되, 원래 작업 스레드의 문맥을 반드시 복원한다. Domain·Application 계약에 MDC 의존성을 추가하지 않는다.

예약 작업처럼 HTTP 요청과 무관한 실행에는 새 요청 ID를 만들지 않는다. SSE 전송을 다른 요청이 유발했다면 그 작업의 로그는 유발한 요청을 따르고, 연결 자체의 수명주기 콜백은 연결 생성 요청을 따른다.

비동기 동작을 변경할 때는 작업 본문뿐 아니라 오류 처리·타임아웃·후속 디스패치의 ID 유지와 작업 스레드 재사용 후 문맥 복원도 검증한다. 타임아웃 응답이 먼저 끝나더라도 작업 본문이 계속 실행 중이라면 작업 스레드의 문맥 정리는 그 작업이 종료될 때 수행된다.

#### 장애 알림 연계

Discord 장애 알림의 `Request-Id` 필드는 요청 처리 중 확정된 MDC 문맥의 ID를 사용한다. 수신 헤더 원문을 다시 읽지 않으며, 요청 문맥이 없는 호출은 `none`으로 표시한다. 알림을 받은 운영자는 이 값으로 같은 요청의 애플리케이션 로그와 Nginx 접근 로그를 검색한다. 백엔드가 ID를 새로 생성한 경우 Nginx 로그와 동일한 ID가 존재한다고 가정하지 않는다.
